...
nombresContenido.push(nombreAtleta)
}
function datoAleatorio(array) {
return array[Math.floor(Math.random()*(nombresContenido.length))]
}
function datosAleatorios(numero, array) {
    let atletasArray=[];
     while(atletasArray.length<numero) {
       let string = datoAleatorio(array);
         if(atletasArray.indexOf(string) == -1) atletasArray.push(string);
    }
    return atletasArray;
  }
let pintarNombre = datosAleatorios(40, nombresContenido);
pintarNombre.forEach(function(element){
    let div = document.createElement("div");
    div.textContent = element
    document.getElementById("descripcionTarjeta").appendChild(div)
})
